...
def callback_reception(call: telebot.types.CallbackQuery):
    ...
    for command in commands:  # Для кастомной обработки
        if data.split('_')[-1] == command:
            ...
            if command == 'send':
                clear_next_step_handler(None, call.message)
                ...
                clear_next_step_handler(None, call.message)
                botMessage = message_correct_answer.line(call.message, deleting_message=False)
                bot.register_next_step_handler(botMessage, correct_answer(botMessage, command_data[0], call.message))
